...
public sealed class PapRewriter(PapRewriter.PapResourceHandlerPrototype papResourceHandler) : IDisposable
{
    ...
    public void Rewrite(string sig, string name)
    {
        ...
        foreach (var hookPoint in hookPoints)
        {
            ...
            WriteToAlloc(stringAllocation, 2048, name);
            ...
            var skipIndex = IndexOf(funcInstructions, instr => instr.IP == hookPoint.IP) + 1;
    ...
    {
        foreach (var (stackAccess, index) in stackAccesses.Select((x, i) => (x, i)))
        {
    ...
    {
        if (!_nativeAllocPaths.TryGetValue((funcAddress, stackRegister, stackDisplacement), out var mem))
        {
    ...
    private static int IndexOf<T>(IEnumerable<T> array, Predicate<T> predicate)
    {
        var i = 0;
